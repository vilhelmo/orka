...
VLCMovieProvider::VLCMovieProvider(const std::string & filename) :
	...
	std::cout << std::endl << std::endl << "Width:" << width << " height: " << height << std::endl
			<< std::endl << std::endl;

	frameData_ = (vlc_int *) malloc(width_ * height_ * sizeof(vlc_int));
	currentFrame_ = new OrkaImage(width_, height_, channels_);
	libvlc_video_set_callbacks(vlc_media_player_, vlc_lock, vlc_unlock, vlc_display, this);
    libvlc_video_set_format(vlc_media_player_, "RGBA", width_, height_, width_*sizeof(vlc_int));
